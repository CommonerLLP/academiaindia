...
| Manual-entry provenance pill | `docs/app.js` adds a "⚑ manual entry" pill on listing cards where `_manual_stub === true` and `_source_method` matches `/manual transcription/i`. |
| Disclaimer pass | Lede simplified to "A public-interest tracker of academic work in India."; meta tags realigned. Caveat below the vacancy-gap banner; scope statement at the top of The Gap; colophon disclaimer paragraphs. |
